...
- **[Toss Payments](https://www.tosspayments.com/)** - 결제 게이트웨이
  - 테스트 모드 결제 지원
  - 다양한 결제 수단 지원
...
### 🛍️ 쇼핑몰 기능
- **상품 관리**: 상품 목록, 상세 페이지, 카테고리 필터링
- **장바구니**: 상품 추가/삭제, 수량 변경
- **주문 관리**: 배송지 입력, 주문 생성, 주문 내역 조회
- **결제 통합**: Toss Payments 테스트 모드 결제
...
- `orders`: 주문 정보 테이블
- `order_items`: 주문 상품 정보 테이블

> **참고**: 마이그레이션 파일은 `supabase/migrations/` 디렉토리에 있습니다.
...
> **⚠️ 주의**: 현재는 테스트 모드로만 운영합니다. 프로덕션 결제는 별도 설정이 필요합니다.
...
2. 환경변수 설정 (자세한 내용은 [환경변수 가이드](./docs/deployment/environment-variables.md) 참고)
3. 배포 실행

자세한 배포 가이드는 [배포 체크리스트](./docs/deployment/checklist.md)를 참고하세요.
